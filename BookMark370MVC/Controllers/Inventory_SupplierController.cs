...
namespace BookMark370MVC.Controllers
{
    public class Inventory_SupplierController : Controller
    {
        ...
        public ActionResult Index(string searchBy, string search)
        {
            if (searchBy == "Name")
            {
                return View(db.Inventory_Supplier.Where(x => x.InvSupp_Name.Contains(search) || search == null).ToList());
            }
            else if (searchBy == "Address")
            {
                return View(db.Inventory_Supplier.Where(x => x.InvSupp_Address.Contains(search) || search == null).ToList());
            }
            else if (searchBy == "Email")
            {
                return View(db.Inventory_Supplier.Where(x => x.InvSupp_Email.Contains(search) || search == null).ToList());
            }
            else
            {
